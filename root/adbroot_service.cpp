...
static inline bool isAutomotiveAdmin(uid_t uid) {
    appid_t appid = multiuser_get_app_id(uid);
    userid_t userid = multiuser_get_user_id(uid);

    return appid == AID_SYSTEM && userid == AUTOMOTIVE_ADMIN_ID && ::android::isAutomotive();
}
...
ndk::ScopedAStatus ADBRootService::isSupported(bool* _aidl_return) {
    ...
    if (uid != AID_SYSTEM && uid != AID_SHELL && !isAutomotiveAdmin(uid)) {
...
ndk::ScopedAStatus ADBRootService::setEnabled(bool enabled) {
    uid_t uid = AIBinder_getCallingUid();
    if (uid != AID_SYSTEM && !isAutomotiveAdmin(uid)) {
